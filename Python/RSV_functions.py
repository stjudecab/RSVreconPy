...
def generate_pdf_report(csv_file, working_folder):
    ...
    df.columns = ['Sample name', 'Pass QC', 'Mapping SubTypeA', 'Mapping SubTypeB', 'Subtype']
    data = df.values.tolist()
    data.insert(0, df.columns.tolist())
    table = Table(data)
    table._argW = [80,60,100,100,80]
    ...
    for cur_folder in Sample_folders:
        ...
        gs2 = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=gs[1], hspace=0.1)
        ...
        ax_genome.set_ylabel('Genes', fontdict=font_axis)
        ...
        plt.savefig(cur_folder + '_fig.png', dpi = 300)
        plt.close()

        img_cov = Image(cur_folder + '_fig.png', width=600, height=300)
