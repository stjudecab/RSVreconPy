...
from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer, Flowable, Table, Image, TableStyle, PageBreak
...
def generate_pdf_report(csv_file, working_folder):
    ...
    color_dict = {'SubtypeA': '#ffcccc', 'SubtypeB': '#ccccff', 'Not RSV': '#d3d3d3'}
    for row in range(1, len(data)):
        for col in [3]:
            value = data[row][col]
    ...
    if os.path.isfile(tree_A_file) or os.path.isfile(tree_B_file):
        ...
        #phylogenetic_text = "Reference sequences from <b>ATCC</b> and <b>BEI</b> are highlighted in <font color='blue'>blue</font>"
        #paragraph = Paragraph(phylogenetic_text, styles['BodyText'])
        #elements.append(paragraph)

        #phylogenetic_text = "Reference sequences (from <b>GISAID</b>) used for mapping are highlighted in <font color='red'>red</font>"
        #paragraph = Paragraph(phylogenetic_text, styles['BodyText'])
        #elements.append(paragraph)
